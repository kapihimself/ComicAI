...
if ($page === 'pos' && $method === 'GET') {
    ...
    $products = $productRepository->activeForPos();
    $settings = $settingsRepository->get();
...
if ($page === 'checkout' && $method === 'POST') {
    ...
    try {
        $user = Auth::user();
        $itemsRaw = json_decode((string) ($_POST['cart_items'] ?? '[]'), true);
        if (!is_array($itemsRaw)) {
            throw new RuntimeException('Format data keranjang tidak valid.');
        }
        ...
        $checkoutData = CheckoutData::fromRequest($_POST, $items, (int) $user['id'], $taxRate);
        $orderId = $checkoutAction->execute($checkoutData);

        Flash::success('Transaksi berhasil!');
        Response::redirect('/?page=receipt&id=' . $orderId);
...
if ($page === 'receipt' && $method === 'GET') {
    ...
    $stmtLines = $pdo->prepare("SELECT * FROM order_lines WHERE order_id = :id");
    $stmtLines->execute([':id' => $id]);
    $orderLines = $stmtLines->fetchAll();
